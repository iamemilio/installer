...
func TestValidateInstallConfig(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			expectedError: `^compute\[0\]\.platform.openstack: Invalid value: openstack.MachinePool{FlavorName:"", RootVolume:openstack.RootVolume{Size:0, Type:""}}: cannot specify "openstack" for machine pool when cluster is using "aws"$`,
		},
		{
